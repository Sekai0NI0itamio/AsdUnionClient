...
object ExperimentalKillAura : Module("ExperimentalKillAura", Category.COMBAT, Keyboard.KEY_H, hideModule = false) {
    ...
    private var blockRange by object : FloatValue("BlockRange", range, 1f..8f) {
        ...
        override fun onChange(oldValue: Float, newValue: Float) = newValue.coerceAtMost(this@ExperimentalKillAura.range)
    }
    ...
    private val swingFails = mutableListOf<ExperimentalSwingFailData>()
    ...
    private fun runAttack(isFirstClick: Boolean, isLastClick: Boolean) {
        ...
        runWithModifiedRaycastResult(rotation, range.toDouble(), throughWallsRange.toDouble()) { raycastResult ->
            val shouldEnterBlockBreakProgress =
                !shouldDelayClick(raycastResult.typeOfHit) || lastAttackTickData?.first?.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK

            if (shouldEnterBlockBreakProgress && isFirstClick) {
            ...
            if (!shouldDelayClick(raycastResult.typeOfHit)) {
                val previousAttackTickData = lastAttackTickData
                lastAttackTickData = raycastResult to runTimeTicks

                if (raycastResult.typeOfHit.isEntity) {
                    val entity = raycastResult.entityHit
                        ...
                        if (entity.hurtTime <= hurtTime) {
                            if (!multi) {
                                ...
                                for (e in world.loadedEntityList) {
                                    val distance = player.getDistanceToEntityBox(e)

                                    if (e is EntityLivingBase && isEnemy(e) && distance <= getRange(e)) {
                                        attackEntity(e, isLastClick)
                    ...
                    if (renderBoxOnSwingFail && raycastResult.typeOfHit.isMiss) {
                        synchronized(swingFails) {
                            ...
                            swingFails += ExperimentalSwingFailData(spot, System.currentTimeMillis())
                        }
                    }
                }
            }
    ...
    private fun updateTarget() {
        ...
        val killAuraTargeter = KillAuraTargeter
    ...
    private fun isEnemy(entity: Entity?): Boolean {
        return isSelected(entity, true) && !isFriend(entity)
    }

    private fun isFriend(entity: Entity?): Boolean {
        return entity is net.minecraft.entity.player.EntityPlayer && Friends.isFriend(entity.name)
    }
    ...
    private fun shouldPrioritize(): Boolean = when {
        !onScaffold && Scaffold.handleEvents() && (Scaffold.placeRotation != null || currentRotation != null) -> true
...
data class ExperimentalSwingFailData(val vec3: Vec3, val startTime: Long)
